<template>
  <div>
    <p class="mt-0 uppercase font-bold text-slate-400 mb-1">Lesson {{ parseInt(chapter.number) }} - {{ parseInt(lesson.number) }}</p>
...
const lesson = computed(() => {
  return chapter.value.lessons.find((lessons) => lessons.slug === route.params.lessonSlug);
});
